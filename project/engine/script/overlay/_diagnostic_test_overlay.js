...
const DIAGNOSTIC_TEST_COPY = Object.freeze({
    ...
    [DIAGNOSTIC_TEST_TYPES.DISPLAY]: 'Apply window, fullscreen, render scale, and ultrawide runtime settings.',
...
export class DiagnosticTestOverlay extends BaseOverlay {
    ...
    #buildBottomControls(handler) {
        ...
        if (this.testType === DIAGNOSTIC_TEST_TYPES.DISPLAY) {
            handler.bottomSpace('WH', 2.1)
                ...
                .item('button').stylePreset('overlay_interact_button').width('content').buttonText('Wide+').buttonColor(ColorSchemes.Overlay.Button.Confirm).icon('confirm').onClick(() => {
                    void this.#applyWidescreenSupport(true);
                })
                .item('button').stylePreset('overlay_interact_button').width('content').buttonText('Wide-').buttonColor(ColorSchemes.Overlay.Button.Confirm).icon('deny').onClick(() => {
                    void this.#applyWidescreenSupport(false);
                })
                .endGroup()
                .bottomSpace('WH', 1.2)
    ...
    #drawDisplayTestContent() {
        ...
        const metrics = [
            ...
            ['Ultrawide', getSetting('widescreenSupport') !== false ? 'enabled' : 'disabled'],
    ...
    async #applyWidescreenSupport(widescreenSupport) {
        this.displayTestStatus = `Applying ultrawide ${widescreenSupport ? 'on' : 'off'}...`;
        try {
            await setSettingBatch({ widescreenSupport });
            await this.systemHandler?.applyRuntimeSettings?.({ widescreenSupport });
            this.systemHandler?.resize?.();
            this.displayTestStatus = `Ultrawide ${widescreenSupport ? 'enabled' : 'disabled'}`;
        } catch (error) {
            console.error(error);
            this.displayTestStatus = error?.message || String(error);
        }
