...
- Drive의 초기 자료에는 업적 이름 외 조건이 미완료였지만 이후 사용자 확정안으로 6개
  설명과 해금 조건을 받았다. 기존 메타의 안정 ID와 멱등 해금을 유지하면서 실제 곡괭이
  벽 통과, 오카리나 획득, 로라에 의한 사망, 2페이즈 진입, 성공 엔딩, 진엔딩 사건을 각각
...
- Turn 16 최종 `npm test` 105개와 PNG 55개·MP3 26개 원본/복사본 검사는 통과했다.
- 지정된 Drive의 기획·회의록·UI 폴더를 확인했다. 아이템 파일명 대응은 자료와 일치했고,
  업적의 구체 규칙은 여전히 미확정이어서 위 임시 정책을 유지했다.
- 제품 버전 `0.108.0`인 NW.js 앱을 1538×900에서 실행해 메뉴, 갤러리, 컷씬, 두 스타터와
  첫 전투 게이지를 확인했다. 네 모서리 클릭과 장시간 층 전환은 여전히 수동 미확인이다.
- 모든 194개 에셋 경로는 `manifests/asset-provenance.json`에 분류됐지만 제공 아트·오디오와
  외부 게임 참조 아이콘은 권리 증빙이 없어 공개 릴리스 차단 상태다.
