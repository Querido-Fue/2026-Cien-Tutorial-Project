...
2. `TutorialBattleAnimationCoordinator.route()`가 배우 동작과 월드 effect를 시작하고 피해 cue를 실제 impact까지 지연한다.
...
| `world-animation` | `effectId`, `animationId`, 선택적 `from`·`to`·`facing` | battle effect animator/view | 플레이어 화살 이동 또는 로라 맵 전역 폭발 시작 |
...
`impactActorId`, `impactAnimationId`, `impactFacing`, `waitForImpact`를 기록한다. 투사체나 폭발이
있는 공격은 `impactEffectId`도 기록하고 배우 프레임 지연과 effect 충격 지연 중 큰 값을
떠오르는 글자·흔들림·플래시·오디오의 `delaySeconds`로 전달한다. 경로 이동·층 전환
...
| `lora-attack` | log, actor-animation(`melee`/`area`/`idle`), area이면 world-animation(`lora-area-explosion`), 비대기 시 공격 audio |
| `lora-damaged` | log, health-transition, 플레이어 공격 audio, 활이면 world-animation(`player-arrow`), impact 지연 floating-text·actor-animation(`hit`/`death`)·shake·flash·hurt/death audio |
| `mob-attack` | 피해 event와 결합해 공격 source actor-animation(`attack`) |
| `mob-damaged` | log, health-transition, 플레이어 공격 시작/audio, 활이면 world-animation(`player-arrow`), impact 지연 floating-text·actor-animation(`hit`/`death`)·shake·flash·slime-hurt audio |
| `mob-defeated` | log |
| `mob-waited` | log |
...
- 화살과 폭발도 같은 가변 프레임으로 진행한다. Pause/Record에서는 clock을 멈추고, reset·destroy는 effect와 ID 세대를 함께 비운다.
- 화살은 발사 프레임 뒤 플레이어 타일에서 대상 타일까지 이동하고 도착 시각을 impact로 사용한다. 폭발은 970×580 프레임 목록을 2D `sourceRect`로 재생하며 과도한 두 번째 전면 플래시는 건너뛴다.
