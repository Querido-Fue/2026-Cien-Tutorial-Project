...
- 웹 발표는 게임 번들과 분리된 `project/presentation/public/ppt/nthplayer/`에 있으며, `PresentationDeck`이 Figma 중간 발표의 편집형 타이포그래피를 따른 15개 장면과 던전 파노라마 카메라를 연속 전환합니다. 동일 출처 `/game/nthplayer/?embed=presentation` iframe은 첫 장부터 한 번만 미리 로드되고, `NthPlayerGameFrame`이 비활성·크기 전환 중에는 합성 canvas 스냅샷을 표시하며 실제 iframe은 마지막 기하에 한 번만 배치합니다. 지원 브라우저에서는 View Transition으로 0.6초 `easeOutExpo` 슬롯↔전체 화면 전환을 수행하고, Escape 복귀와 `window.nthPlayerPresentation` API를 유지합니다. Cloudflare Worker는 발표 쿼리의 게임 HTML에만 `embed-input-bridge.js`를 부트스트랩보다 먼저 주입해 임베드 입력을 활성화하며, `PRESENTATION_ASSETS` 바인딩이 `/ppt/nthplayer/` 셸과 분리된 기본/시스템 슬라이드 CSS를 제공합니다.
...
- `TutorialBattleOutcomeCoordinator`는 모델 결과를 presenter→battle animation coordinator→feedback/timeline→업적/메타→기록→컷씬 트리거 순서로 배포하고 이전 표현 snapshot을 소유합니다. 화면 전환·층 교체는 장면에 남기며, 조정자는 장면 전체가 아닌 명시적 구독자와 투영 포트만 받습니다.
- `TUTORIAL_CONTENT_DATA`가 확정 업적명·설명·해금 조건, 일기 순서와 안정 기록 ID, 엔딩 표시명과 남은 미확정 상태를 소유합니다. `TutorialAchievementEvaluator`는 안정된 모델 사건으로 확정 조건을 판정하고 배너는 알림 수명만, `TutorialGalleryController`는 섹션·항목 선택과 메타 기반 열람 상태만 맡습니다. 메뉴 갤러리 탐색은 `TutorialGalleryNavigationController`가 선택과 전환을 조율하고 `TutorialGalleryPageTurnController`가 0.7초 진행도·이전 선택 상태만 소유합니다. `TutorialGalleryContentView`가 책·글·그림을 동일한 렌더 계약으로 그리고, `TutorialGalleryPageTurnSurface`가 draw 단계에 이전·다음 책을 두 오프스크린 캔버스로 한 번씩 래스터화합니다. 입력 전에 `frame.clear`가 실행되므로 현재 `ui` 픽셀을 캡처하지 않습니다. `PageTurnEffectPass`는 이전 출발 면·다음 목적 면을 같은 책등 기준의 양면 3D 곡면 메시로 넘기며 고정 면·원근·깊이·낙하 그림자도 함께 합성합니다. WebGL 제출 중에는 평면 2D 책·본문을 중복해서 그리지 않고, 초기화/래스터 실패·context loss에서는 같은 프레임에 기존 책 PNG 폴백을 복구합니다. `TutorialRecordPopupQueue`는 한 경로에서 여러 기록을 획득한 경우의 순서만, `TutorialRecordPopupController`는 대기열과 0.6초 `easeOutExpo` 진입·0.4초 `easeInExpo` 퇴장 수명주기를 조율합니다. 기록 모드에서는 전투 장면을 유지한 채 `vignette` 경계에 맞춘 전용 DOM backdrop으로 블러·감광하고 책·버튼은 `top` 레이어에서 선명하게 그립니다. 곡괭이 업적은 획득이 아닌 실제 `wall-traversed`, 최초 사망 업적은 `battle-finished.defeatedBy === 'lora'`, 2페이즈 업적은 `floor-transition.floorIndex === 1`을 사용합니다.
- `TutorialRecordSpawnPlanner`는 새 런마다 영구 해금되지 않은 10개 기록 풀에서 중복 없이 최대 두 ID를 뽑고, 각 층의 기존 다섯 후보 좌표 중 하나를 별도로 추첨해 페이즈당 최대 한 개만 배치합니다. 남은 기록이 한 개면 1페이즈에만, 모두 해금했으면 어느 층에도 배치하지 않으며 실제 런은 `Math.random`, 테스트·밸런스 하네스는 주입한 고정 난수를 사용합니다.
...
- 전투 화면도 `TutorialBattleWorldView`, `TutorialBattleActorView`, `TutorialBattleEffectView`, `TutorialBattleHudView`, `TutorialBattleCommandMenuView`, `TutorialBattleFeedbackView`, `TutorialAchievementView`, `TutorialBattleTutorialView`의 파일당 한 클래스로 나뉩니다. 월드 뷰는 오브젝트 정렬을, 배우 뷰는 플레이어·로라·슬라임의 기하·그림자·도형 폴백을, `TutorialSpriteFrameRenderer`는 몸·머리 레이어의 정수 좌표 합성을, effect 뷰는 화살 WebGL과 대형 폭발 2D crop을 소유합니다. 커맨드 메뉴 뷰는 이동 확정↔공격·대기·회복의 배치·플립 표현·동일 히트 영역을 소유합니다. `TutorialScene`이 한 프레임의 `BattleViewModel`을 조립하고, 이미지 객체는 `TutorialAssetPort`로 별도 주입합니다. 모델 미리보기의 표시 변환은 `TutorialCombatReadabilityPresenter`, 공통 조사 포커스는 `TutorialBattleFocusController`, 안내의 단일 단계·expo 전환 상태는 `TutorialGuidanceController`, UI와 `top` 사이의 선택 영역 아웃포커스 DOM은 `TutorialGuidanceBackdropView`가 맡습니다.
- `TutorialBattleLayout`이 보드·HUD 기하와 타일 투영·히트테스트를 함께 소유합니다. 맵 아트가 있으면 매니페스트의 원본 970×580 격자 네 꼭짓점으로 9×8 축을 만들고 실제 격자의 좌우 폭을 월드 뷰포트에 맞춥니다. 맵 프로필의 `ambientFire` 심지 좌표도 같은 `mapImageRect`로 화면에 투영합니다. `TutorialBattleLightingView`는 두 심지를 촛대 하나의 광원으로 묶고 맵 중심 점대칭의 반대편 가상 벽 광원을 더 약하게 만들며, `SceneLightingEffectPass`가 스크린 합성·국소 scissor 원형 감쇠와 불꽃 위상 기반 미세 호흡으로 월드만 비춥니다. 지하는 노출 0.6, 청색 암부와 강화된 저밀도 부유 입자를 사용합니다. 실제 심지의 난류 화염·코어·불씨는 `FlameParticleEffectPass`가 별도 합성합니다. `TutorialBattleCameraController`는 가상 게임 커서의 화면 가장자리 침투량 역투영, 휠 클릭 중앙 복귀와 누적 휠 목표 줌을 소유합니다. `TutorialBattleCamera`는 추적점을 0.3초 감쇠로 따라가고, 진행 중 목표를 현재 배율에서 재지정하는 0.4초 `easeOutExpo` 줌을 적용합니다. 최소 줌은 맵 이미지 좌우가 뷰포트에 닿는 동적 배율이고 최대 줌은 기본의 1.2배이며, 렌더와 입력 판정은 같은 카메라 snapshot이 적용된 layout frame을 사용합니다.
- 정적 `world-postprocess` WebGL surface는 `effect`와 `texteffect` 사이에 위치합니다. `WorldPostProcessPipeline`이 `background`·`object`·`effect` 명령을 레이어 순서대로 하나의 nearest-filtered FBO에 지연 합성하고, 공격 충돌 때만 두 번째 full-resolution nearest FBO의 `SpatialDistortionPostProcessPass`로 최대 4개의 방사형 굴절 링을 적용한 뒤 1/4 해상도 Bloom·색보정·디더/그레인·비네팅을 적용합니다. `spatialDistortion`은 일반 effect draw가 아니라 파이프라인 전용 명령이며, `texteffect`·`ui`·오버레이는 입력에서 제외합니다. WebGL 초기화·프레임 오류나 context loss에서는 같은 프레임의 일반 명령을 기존 세 surface에 재생하고 전용 명령은 안전하게 생략합니다. 절차적 화염·마그네틱 실드·인게임 먼지는 화면 좌표를 2픽셀 격자에 스냅하고, 먼지는 `effect` 레이어의 제한된 point sprite로 보드 위·UI 아래에 합성하며 Bloom만 선형 샘플링합니다.
- 사용자 제공 PF 스타더스트 S/Bold/ExtraBold는 `PFStardust` 패밀리의 400/700/800 굵기로 등록하고, 시작 시 `TEXT_RENDER_DATA.FONT_FACES`를 통해 `FontFaceSet.load()` 완료를 기다립니다. 본문은 S, 강조·버튼은 Bold, 제목·핵심 정보는 ExtraBold를 사용합니다. 폰트 자체가 도트 디자인이므로 텍스트는 Canvas 기본 `fillText()`·`measureText()` 경로로 렌더링하며, `_pixel_text_renderer.js` 구현은 보존하되 런타임 그리기 경로와 연결하지 않습니다.
...
- 모델 event 해석은 `TutorialBattlePresenter`, cue 순서·일시 피드백·오디오 drain 대기열은 `TutorialFeedbackQueue`, 표시 보간·animation slot은 `TutorialAnimationTimeline`이 각각 소유합니다. 배우 클립 폴백은 `TutorialSpriteClipResolver`, 델타 기반 재생·중단·프레임 이벤트는 `TutorialSpriteAnimator`, 표시 층 배우 투영은 `TutorialSpriteRoster`, 배우/effect impact 지연과 발걸음 cue 파생은 `TutorialSpriteCueRouter`, 화살·폭발 수명은 `TutorialBattleEffectAnimator`가 맡습니다. `TutorialAttackDistortionAnimator`는 같은 target-hit cue에서 근접 공격의 배우 impact와 원거리 화살 도착 시점을 읽어 확산 왜곡 수명만 만들고, `TutorialBattleAnimationCoordinator`가 이들을 조립합니다. 입력 잠금과 결과 진입은 timeline과 coordinator가 모두 idle일 때만 풀립니다. `TutorialAudioDirector`는 작은 장면 상태와 drain된 cue를 전역 sound port로 변환합니다. 이 모듈들은 모델과 장면을 역참조하지 않으며, 동일 event를 뷰나 장면에서 다시 해석하지 않습니다.
- `TUTORIAL_ASSET_MANIFEST`와 `TUTORIAL_AUDIO_MANIFEST`가 `project/asset` 원본 이름·안전한 런타임 경로·필수 여부·폴백을 소유하며, 이미지 계약은 맵·UI·아이템·스프라이트·전투 effect별 크기·레이어·용도, 오디오 계약은 bus·loop·gain·polyphony·cooldown을 추가로 선언합니다. `scripts/import-tutorial-assets.mjs`는 원본을 수정하지 않고 `project/asset/tutorial/`로 ASCII 이름 복사하며 `scripts/check-assets.mjs`가 PNG IHDR와 MP3 헤더·해시·충돌을 검사합니다.
...
- `TUTORIAL_SPRITE_CLIPS`는 32×32 논리 프레임, 배우 앵커, 프레임별 좌·우 발 그림자 접점, source rect 레이어와 `offsetXRatio/offsetYRatio/castsShadow`, FPS, 루프, impact 프레임과 명시적 폴백을 데이터로 소유합니다. 플레이어·로라 좌우 idle은 몸을 고정하고 머리만 1픽셀 올리는 4프레임 호흡 합성이며 머리와 검격은 그림자를 만들지 않습니다. 배우 그림자는 두 격자축을 합친 동남쪽 평면 투영을 사용하고 실측한 두 발에서 실루엣을 시작합니다. WebGL 이미지 명령은 선택적 `sourceRect`와 축 반전을 정규화 UV로 바꾸며, 픽셀 스프라이트는 정수 사각형과 nearest-neighbor로 그립니다. 대형 폭발 시트는 WebGL에 올리지 않고 Canvas 2D 9인자 `drawImage`의 `sourceRect`로 현재 프레임만 렌더합니다.
