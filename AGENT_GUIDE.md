...
- `TutorialInventoryPresenter`는 인벤토리 페이지·아이템 공개/사용 가능 표시를, `TutorialBattleViewModelFactory`는 모델의 읽기 API와 선택·표현 snapshot을 최종 BattleViewModel로 조립하는 책임을 맡습니다. `TutorialScene`은 HUD용 아이템 메타나 행동 미리보기 표시값을 다시 계산하지 않습니다.
