...
- 웹 발표는 게임 번들과 분리된 `project/presentation/public/ppt/nthplayer/`에 있으며, `PresentationDeck`이 15개 장면과 한 장의 던전 파노라마 카메라를 연속 전환합니다. 동일 출처 `/game/nthplayer/` iframe은 첫 장부터 한 번만 미리 로드되고, `NthPlayerGameFrame`과 `window.nthPlayerPresentation` API가 재로드 없는 슬롯·전체 화면 위치/크기/불투명도 전환을 소유합니다. 프로토타입 전체 화면은 0.6초 `easeOutExpo`, Escape 복귀 계약을 가지며, Cloudflare Worker의 `PRESENTATION_ASSETS` 바인딩이 `/ppt/nthplayer/`에서 제공합니다.
...
- 사용자 제공 `OwnglyphParkDahyun.ttf`는 시작 시 `FontFaceSet.load()` 완료를 기다린 뒤 사용합니다. `TEXT_RENDER_DATA`가 대상 패밀리·도트 격자·알파 임계값·LRU 상한을 선언하고, `PixelTextRenderer`가 텍스트만 저해상도 Canvas에 캐시해 최근접 확대하며 `measureText()`도 같은 래스터 메트릭을 사용합니다. 다른 폰트와 비텍스트 UI는 기존 렌더 경로를 유지합니다.
