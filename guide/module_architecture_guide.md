...
-> object(WebGL)
-> effect(WebGL)
-> world-postprocess(WebGL)
-> texteffect(2D)
-> ui(2D)
-> tutorial-guidance-focus-backdrop(DOM, 안내 중에만)
-> vignette(2D)
-> top(2D)
...
- `TutorialCutsceneController`는 `TUTORIAL_GAME_DATA.CUTSCENES`의 고정 카드만 진행하고, `TutorialCutsceneSession`은 한 런의 중복 방지·대기열·복귀 모드를 소유합니다. `TutorialCutsceneTriggerRouter`는 첫 실행 메타와 `item-used`, `floor-transition`, `battle-finished` 같은 실제 사건을 감사된 컷씬 ID로만 변환하며 전투 모델에 대화 phase를 추가하지 않습니다. AI 기반 채팅이나 생성형 대화 경로도 없습니다.
